...
attach_active_cpl!(sys_dict["Marconato"])
attach_active_cpl!(sys_dict["VSM"])
attach_active_cpl!(sys_dict["Droop"])
attach_active_cpl!(sys_dict["dVOC"])
...
# Paper Figure
plot(P_load, V_load, label = "PV Curve", xlabel = "Load Power [pu]", ylabel = "Load Bus Voltage [pu]", color = :black)
genrou_ixs = dict_true_ixs["Marconato"]
droop_ixs = dict_true_ixs["Droop"]
